...
function getScriptPath() {
  const scripts =
    _doc.currentScript ||
    (function() {
      const s = _doc.getElementsByTagName("script");
      return s.length ? s[s.length - 1] : false;
    })();
  const path = scripts ? scripts.src.split("?")[0] : "";
...
if (!setAnimationFrame) {
  ...
  setAnimationFrame = function(callback, element) {
    ...
    const id = _win.setTimeout(function() {
...
const getBrowserDetection = function() {
  if (browserdetected) return browserdetected;

  let _el = _doc.createElement("DIV");
  const _style = _el.style;
  ...
  d.hastransition =
    ...
    (function() {
      ...
      const prefix = ["-webkit-", "-ms-", "-moz-", "-o-", "-o", "-khtml-"];
      const evs = [
        "webkitTransitionEnd",
        "msTransitionEnd",
        "transitionend",
...
const NiceScrollClass = function(myopt, me) {
  let self = this;
  ...
  if (opt.rtlmode == "auto") {
    const target = this.win[0] == _win ? this.body : this.win;
    const writingMode =
  ...
  const cap = $.extend({}, this.detected);
  ...
  this.debounced = function(name, fn, tm) {
    if (!self) return;
    const dd = self.delaylist[name] || false;
    if (!dd) {
      self.delaylist[name] = {
  ...
  const BezierClass = function(st, ed, spd, p1, p2, p3, p4) {
    this.st = st;
    this.ed = ed;
    this.spd = spd;
  ...
  if (this.ishwscroll) {
    ...
    this.notifyScrollEvent = function(el) {
      const e = _doc.createEvent("UIEvents");
      e.initUIEvent("scroll", false, false, _win, 1);
      e.niceevent = true;
      el.dispatchEvent(e);
    };

    const cxscrollleft = this.isrtlmode ? 1 : -1;
  ...
  this.getOffset = function() {
    if (self.isfixed) {
      const ofs = self.win.offset(); // fix Chrome auto issue (when right/bottom props only)
      const scrl = self.getDocumentScrollOffset();
      ofs.top -= scrl.top;
      ofs.left -= scrl.left;
      return ofs;
    }
  ...
  this.updateScrollBar = function(len) {
    ...
    if (self.ishwscroll) {
      ...
      if (self.railh && !self.railslocked) {
        ...
        const y = self.railh.align
          ? pos.top +
            getWidthToPixel(self.win, "border-top-width", true) +
            self.win.innerHeight() -
            self.railh.height
          : pos.top + getWidthToPixel(self.win, "border-top-width", true);
        const x = pos.left + getWidthToPixel(self.win, "border-left-width");
        self.railh.css({
  ...
  this.init = function() {
    ...
    const _scrollyhidden = { "overflow-y": "hidden" };
    ...
    if (!self.ispage || !cap.isieold) {
      ...
      let v;
      let a;
      const kp = ["left", "right", "top", "bottom"]; //* *
      for (const n in kp) {
        a = kp[n];
      ...
      if (self.ispage) {
        ...
        if (self.ishwscroll) {
          ...
          const bd = self.win[0].nodeName == "HTML" ? self.body : self.win;
          $(bd)
            .scrollTop(0)
          ...
          const rlpos = self.isfixed ? "fixed" : "absolute";

          if (!self.isfixed) self.viewport = self.getViewport(self.win[0]);
          if (self.viewport) {
            self.body = self.viewport;
            if (!/fixed|absolute|relative/.test(self.viewport.css("position")))
      ...
      if (
        ...
        self.ontouchstart = function(e) {
          ...
          if (!self.railslocked) {
            ...
            if (tg) {
              const skp = /INPUT/i.test(tg.nodeName) && /range/i.test(tg.type);
            ...
            if (self.ispage || !opt.directionlockdeadzone) {
              ...
              const page = self.getContentSize();

              const maxh = page.h - view.h;
              const maxw = page.w - view.w;
            ...
            if (tg && ismouse) {
              const ip = /INPUT|SELECT|BUTTON|TEXTAREA/i.test(tg.nodeName);
              if (!ip) {
        ...
        const moveneedoffset =
          opt.emulatetouch && self.isiframe && !cap.hasmousecapture;

        const locktollerance = (opt.directionlockdeadzone * 0.3) | 0;
        ...
        self.ontouchmove = function(e, byiframe) {
          ...
          if (self.rail.drag.pt == 2) {
            ...
            const my = fy - self.rail.drag.y;
            let fx = e.clientX + ofx;
            const mx = fx - self.rail.drag.x;
            ...
            if (!self.hasmoving) {
              ...
              const dz = opt.directionlockdeadzone;
      ...
      if (cap.cantouch || opt.emulatetouch) {
        ...
        var checkSelectionScroll = function(e) {
          ...
          const rt = -((self.selectiondrag.df * 2) / 6) | 0;
          self.doScrollBy(rt);
      ...
      self.onkeypress = function(e) {
        ...
        if (tg && /INPUT|TEXTAREA|SELECT|OPTION/.test(tg.nodeName)) {
          const tp = tg.getAttribute("type") || tg.type || false;
      ...
      if (cap.ischrome && !self.ispage && !self.haswrapper) {
        ...
        const tmp = self.win.attr("style");
        const ww = parseFloat(self.win.css("width")) + 1;
            ...
            self.observerremover = new ClsMutationObserver(function(mutations) {
              mutations.forEach(function(mo) {
                if (mo.removedNodes.length > 0) {
                  for (const dd in mo.removedNodes) {
  ...
  this.onResize = function(e, page) {
    ...
    const premaxh = self.page.maxh;
    const premaxw = self.page.maxw;
    const previewh = self.view.h;
    const previeww = self.view.w;
    ...
    self.page = page || self.getContentSize();
    ...
    if (
      ...
      // test position
      if (!self.ispage) {
        const pos = self.win.offset();
        if (self.lastposition) {
          const lst = self.lastposition;
  ...
  this.onscreenresize = function(e) {
    ...
    const hiderails = !self.ispage && !self.haswrapper;
  ...
  this.mousewheel = function(dom, fn, bubble) {
    // bind mousewheel
    const el = "jquery" in dom ? dom[0] : dom;
  ...
  this.undelegate = function(dom, name, fn, bubble, active) {
    const de = delegatevents[name] || false;
    if (de && de.l) {
      // quick fix #683
  ...
  function doScrollRelative(px, py, chkscroll, iswheel) {
    ...
    if (py || px) {
      self.synched("relativexy", function() {
        const dty = self.lastdeltay + self.newscrolly;
        self.lastdeltay = 0;

        const dtx = self.lastdeltax + self.newscrollx;
        self.lastdeltax = 0;
  ...
  function execScrollWheel(e, hr, chkscroll) {
    ...
    const chk = doScrollRelative(px, py, chkscroll, true);
  ...
  if (!opt.smoothscroll) {
    ...
    this.prepareTransition = function(dif, istime) {
      const ex = istime ? dif : self.getTransitionSpeed(dif);
      const trans = `${ex}ms`;
    ...
    this.doScrollPos = function(x, y, spd) {
      ...
      const dd = Math.sqrt(dst.x * dst.x + dst.y * dst.y) | 0;

      const ms = self.prepareTransition(dd);
    ...
    this.doScrollPos = function(x, y, spd) {
      ...
      const ms = self.getTransitionSpeed(dd);

      self.bzscroll = {};

      const p3 = clipped ? 1 : 0.58;
      ...
      var loop = function() {
        ...
        const x = self.bzscroll.y.getPos();
...
var ScrollMomentumClass2D = function(nc) {
  ...
  this.doMomentum = function(gp) {
    ...
    const pageh = self.nc.page.maxh;
    const pagew = self.nc.page.maxw;
...
const _scrollTop = jQuery.fn.scrollTop; // preserve original function
...
const _scrollLeft = jQuery.fn.scrollLeft; // preserve original function
...
const NiceScrollArray = function(doms) {
  const self = this;
  this.length = 0;
...
const pseudos = jQuery.expr.pseudos || jQuery.expr[":"]; // jQuery 3 migration
...
$.fn.niceScroll = function(wrapper, _opt) {
  ...
  const ret = new NiceScrollArray();

  this.each(function() {
    const $this = $(this);

    const opt = $.extend({}, _opt); // cloning
